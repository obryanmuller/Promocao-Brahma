...
function renderTeamSpotlight() {
  ...
  teamSpotlight.innerHTML = `<div><span class="eyebrow eyebrow-dark"><span></span> CLUBE EM DESTAQUE</span><h2>${escapeHtml(team.name)}</h2><p class="spotlight-streak">${beerIcon} <strong>${streak} vitórias consecutivas</strong></p><p class="spotlight-progress">${Array.from({ length: 5 }, (_, index) => `<span class="${index < streak ? 'active' : ''}"></span>`).join('')}</p><p>${missing ? `<strong>Faltam ${missing} vitórias para a Brahma grátis.</strong>` : '<strong>A Brahma é nossa!</strong>'}</p><small>Último resultado: ${escapeHtml(team.lastResult)} · Último adversário: ${escapeHtml(team.nextOpponent)}</small>${fixtureMarkup}</div><button id="share-team" class="button button-yellow" type="button"><span>Compartilhar clube</span><i data-lucide="share-2" aria-hidden="true"></i></button>`;
